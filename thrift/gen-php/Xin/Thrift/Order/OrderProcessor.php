...
class OrderProcessor {
  ...
  protected function process_delGoodsFromCart($seqid, $input, $output) {
    ...
    if ($bin_accel)
    {
      $args = thrift_protocol_read_binary_after_message_begin($input, '\Xin\Thrift\Order\Order_delGoodsFromCart_args', $input->isStrictRead());
    }
    else
    {
    ...
    try {
      $result->success = $this->handler_->delGoodsFromCart($args->userId, $args->goodsId);
    } catch (\Xin\Thrift\Order\ThriftException $ex) {
      $result->ex = $ex;
    }
    $bin_accel = ($output instanceof TBinaryProtocolAccelerated) && function_exists('thrift_protocol_write_binary');
